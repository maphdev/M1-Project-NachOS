...
"-c tests the console\n"
#ifdef CHANGED
"-sc tests the console in synchronous mode\n"
#endif
#endif
#ifdef FILESYS
"FILESYS\n"
...
"-o runs a simple test of the Nachos network software"
    ...
    if (argc == 1)
        SynchConsoleTest (NULL, NULL);          // once we start the console, then
    else                                        // then NachOS will loop forever waiting
      {                                         // for console input
          ASSERT (argc > 2);
          SynchConsoleTest (*(argv + 1), *(argv + 2));
          argCount = 3;
      }
      interrupt->Halt ();                       // machine halting after ctrl-D
      }
    #endif
#endif // USER_PROGRAM
#ifdef FILESYS
